function LegacyDSUFactory(factoryOptions) {
    ...
    const storeDSUInstance = (legacyDSU, callback) => {
        legacyDSU.getKeySSIAsObject((err, keySSIInstance)=>{
            if (err) {
                return callback(err);
            }

            keySSIInstance.getAnchorId((err, anchorId)=>{
                if (err) {
                    return callback(err);
                }

                dsuIdentityExtension.put(anchorId, legacyDSU);
    ...
    this.create = (keySSI, options, callback) => {
        barFactoryInstance.create(keySSI, options, (err, barInstance) => {
            ...
            storeDSUInstance(legacyDSU, callback);
        })
    }
