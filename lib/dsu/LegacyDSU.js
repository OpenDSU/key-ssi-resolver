function LegacyDSU(bar, dsuInstancesRegistry) {
    ...
    let _beginBatch = (callback) => {
        ...
        if(dsuInstancesRegistry.isLocked(anchorId)){
            dsuInstancesRegistry.waitUntilCanBeginBatch(anchorId, initBatch);
            return;
        }
    ...
    this.safeBeginBatch = (wait, callback) => {
        ...
        if (dsuInstancesRegistry.batchInProgress(this.getAnchorIdSync())) {
            if (wait) {
                dsuInstancesRegistry.waitUntilCanBeginBatch(bar.getAnchorIdSync(), _safeBeginBatch);
                return;
            }
    ...
    this.onCommitBatch = (notify, once)=>{
        ...
        listeners.add(new WeakRef({notify, once}));
    }

    this.notifyOnCommitBatch = ()=>{
        listeners.forEach((listener)=>{
            let observer = listener.deref();
            if(observer){
