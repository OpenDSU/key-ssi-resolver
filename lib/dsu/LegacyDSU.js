...
function LegacyDSU(bar, dsuInstancesRegistry) {
    ...
    const parentDSUIsInBatchMode = async () => {
        const keySSI = await $$.promisify(this.getKeySSIAsString)();
        ...
        for (let dsuInstance of instances) {
            if (dsuInstance.batchInProgress()) {
                ...
                for (const mountedDSU of mountedDSUs) {
                    if (mountedDSU.identifier === keySSI) {
                        return true;
                    }
                }
            }
    ...
    this.safeBeginBatch = (wait, callback) => {
        ...
        atLeastOneMountedDSUIsInBatchMode().then(async (isMountedDSUInBatchMode) => {
            if (isMountedDSUInBatchMode) {
                return callback(Error("At least one mounted DSU is in batch mode"));
            }

            if (await parentDSUIsInBatchMode()) {
                return callback(Error("Parent DSU is in batch mode"));
            }
