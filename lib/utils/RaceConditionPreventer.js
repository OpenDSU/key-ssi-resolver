...
function RaceConditionPreventer() {
    ...
    self.waitUntilCanBeginBatch = (anchorId, continuation)=> {
        if(!continuations[anchorId]){
            continuations[anchorId] = [];
        }
        continuations[anchorId].add(continuation);
    }

    let locks = {};
    self.lockAnchorId = (anchorId) => {
        if(locks[anchorId]){
    ...
    self.unlockAnchorId = (anchorId) => {
        ...
        if(possibleContinuations && possibleContinuations.length > 0){
            let continuation = possibleContinuations.shift();
            if(typeof continuation !== "function"){
                throw new Error(`Failed to execution continuation because is not a function: `, continuation);
            }
            continuation();
        }
    ...
    async function dumpBatchInfo(){
        ...
        for(let anchorId of anchorIds){
            ...
            let anchor = require("opendsu").loadApi("keyssi").parse(anchorId).getIdentifier(true);
            let type = "";
            switch (anchorId){
                case mainAnchorId:
                    type = "MAINDSU";
            ...
            console.log(`\tAnchor ${anchor} [${type}]: ${noInstances} instances and ${noInstancesInBatch} in batch`);
        }
    }
